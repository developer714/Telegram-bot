...
class MockWallet:
    ...
    # def send_transaction(self, amount, extra_data):
    #     print(f"Sending {amount} TON with extra data: {extra_data}")
...
db_config = {
    'user': 'root',
    'password': 'nid20000714!!)%',
    'host': '127.0.0.1',
    'database': 'ton_bot',
}

# TON wallet setup
ton_wallet = Wallet('delay warface quote firm void volcano away license time tackle version bar reject tell damp verb limb now market cry mutual battle')
...
async def confirm_transaction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        recipient_address = 'UQBU61PW1hoLoJ4-yJkMRtrTsZD7vhfyfe25jDtDyaXEPpwz'

        payment_link = f"<WALLET_PROTOCOL>://send?recipient={recipient_address}&amount={amount}&data={extra_data}"
...
def check_transactions(smart_contract_address, last_checked_tx):
    # Fetch transactions for the smart contract
    response = requests.get(f'{TON_API_URL}getTransactions', params={
    ...
    for tx in transactions:
        if tx['utime'] > last_checked_tx:
            new_transactions.append(tx)
    
    return new_transactions

async def monitor_contract(update, context):
    last_checked_tx = int(time.time())  # Initialize with current timestamp
    while True:
        new_txs = check_transactions(WALLET_ADDRESS, last_checked_tx)
        for tx in new_txs:
            # Process each new transaction
            # Here, potentially notify users or log the transaction
            await context.bot.send_message(chat_id='CHAT_ID', text=f"New transaction received: {tx}")
      
        last_checked_tx = int(time.time())
...
# Handler for transaction amount input
async def option_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        query = update.callback_query
        instance_id, option_name = query.data.split('_')[1:]
...
async def amount_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            ...
            await update.message.reply_text("Invalid amount. Please enter a valid amount (min 0.1 TON).")
            return
        balance = 100  # Replace with actual balance fetching logic
        if amount > balance:
            await update.message.reply_text(f"Insufficient balance. Your balance is {balance} TON.")
            return
...
async def extra_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        extra_data = update.message.text
        amount = context.user_data['amount']
        instance_id = context.user_data['instance_id']
...
async def confirm_transaction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        group_id = group_selection.get(query.from_user.id)

        if not amount or not group_id or not instance_id or not option_name:
            await query.message.reply_text("Transaction data missing. Please start over.")
            return

        # Mock transaction - replace with actual transaction logic
        # ton_wallet.send_transaction(amount, extra_data)
        
        # Assuming you connect to wallet API to process the transaction

        db_conn = get_db_connection()
        cursor = db_conn.cursor()
        cursor.execute(
            "INSERT INTO transactions (user_id, username, group_id, instance_id, option_name, amount, extra_data) "
            "VALUES (%s, %s, %s, %s, %s, %s, %s)",
            (query.from_user.id, query.from_user.username, group_id, instance_id, option_name, amount, extra_data)
        )
        db_conn.commit()
        cursor.close()
        db_conn.close()

        await query.message.reply_text("Transaction completed successfully!")
